...
def process_dls_files(directory):
    ...
    for filename in os.listdir(directory):
            ...
            #Ajustar eje X
            tau = int(filename[1:].replace(".INT", ""))
            x_valuesnorm = [(x-1)*tau for x in x_values]
            ...
            if directory == "muestras":
                ...
                i=i+1
            # Añadir la curva y la etiqueta para ordenarlas después, seleccionar si quieres plotear g1 o g2
            #g2
            #curves.append((x_valuesnorm, y_values_normalized, label_number))
    ...
    for curve in curves:
        ...
        ax.plot(x_valuesnorm, g1_values, label=f'{label} $\mu$s', color = colores[i])
        i=i+1

    # Configurar la gráfica
    ax.set_xlabel(r'$\tau (\mu s)$',fontsize=14)
    #Cambiar dependiendo de si quieres plotear g1 o g2
    #g1
    ...
    ax.set_title(r'$\theta=90$°, Duración $40$ s, 256 canales, Muestra 8', fontsize=14)

    #plt.show()
    plt.savefig("tiempomuestreo.png", format='png', dpi=500, bbox_inches = 'tight')

# Ejemplo de uso:
process_dls_files("tau")
